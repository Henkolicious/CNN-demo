...
image_width = 100
number_of_epochs = 10
number_of_classes = len(glob.glob(testing_directory + "/*"))
batch_size = 65 # must be 1 or greater 
number_of_images = len(glob.glob(testing_directory + "/crayfish/*")) + \
                    len(glob.glob(testing_directory + "/elephant/*")) + \
                    len(glob.glob(testing_directory + "/flamingo/*")) + \
                    len(glob.glob(testing_directory + "/hedgehog/*")) + \
                    len(glob.glob(testing_directory + "/kangaroo/*")) + \
                    len(glob.glob(testing_directory + "/leopards/*"))

number_of_train_images = len(glob.glob(testing_directory + "/crayfish/*")) + \
                        len(glob.glob(testing_directory + "/elephant/*")) + \
                        len(glob.glob(testing_directory + "/flamingo/*")) + \
                        len(glob.glob(testing_directory + "/hedgehog/*")) + \
                        len(glob.glob(testing_directory + "/kangaroo/*")) + \
...
def augmentImages(classifier):
    ...
    classifier.fit_generator(
        ...
        validation_steps=number_of_train_images)
...
def classifyTestingImages(classifier, training_set):
    ...
    print("Number of predicted images: \t", number_of_images)
    print("Number of trained images: \t", number_of_train_images)
